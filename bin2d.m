function bin=bin2d(x,y,d,X,Y,asc,err)
...
%Time-stamp:<Last updated on 14/10/29 at 16:40:18 by even@nersc.no>
...
for i=1:M % loop through M rows/along column ( y = i = | )
  ...
  if isempty(err)
    ...
    biny=bin1d(x(index),d(index),XG*j,asc);
  else
    %[biny,py]=bin1d(x(index),d(index),XG*j,err(index));
    biny=bin1d(x(index),d(index),XG*j,asc,err(index));
  end

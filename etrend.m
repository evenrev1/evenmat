...
%	  For detrending only, set f=1. Monthly data only!
%       OR
%         vector of the times for x in serial days. Results in pure
%         detrending (not month by month) but handles unevenly
%         sampled data. Fits on full number of years, to avoid
%         seasonal influence on trend.
%
...
if length(f)>1 % JUST PLAIN POLYFIT ON THE TIME SERIES.
  t=f;
  [Y1,M1,D1]=datevec(t(1));  [Y2,M2,D2]=datevec(t(end));
  % A number of full years to do the polyfit on:
  if M2==M1-1 | M2==12&M1==1,	i=1:M;
  elseif M2>=M1 | M1==12&M2<11,	i=find(t<datenum(Y2,M1,D1)); 
  else				i=find(t<datenum(Y2-1,M1,D1));
  end
  disp(['Taking trend from ',datestr(t(i(1))),' to ',datestr(t(i(end))),'.']);
  Mf=length(i);
  y=x;
  ...
  I=~isnan(y(i,:)); % logical for non-nans in rows
  yl=nan(M,NN); 
  P=nan(degree+1,NN);
  Q=nan(5,NN);
  for j=find(~all(~I))
  ...
  yl=reshape(yl,[M,D]);		% Reshape matrix into array
  %
  y=x-yl;			% Finally detrend (subtract)
  %
  Pm=size(P,1);
  P=reshape(P,[Pm,D]);		% Reshape matrix into array
  Qm=size(Q,1);
  Q=reshape(Q,[Qm,D]);		% Reshape matrix into array
  %
  % a plot
  if nargout==0 & D==1
    %x=x(:,:);y=y(:,:);
    ...
    plot(t,x,'k-o',t,yl,'g-*');
    text(t(end),yl(end),num2str(squeeze(P(1,:,:,:)),'%5.2f'),'color','g','horizontalalignment','left');
    %plot(t,y,'.',tt,yy,'r-','linewidth',2);
    %legend('data input',...
    %	 'detrended and deseasoned series',...
    %	 'The ''trendline''/sequence of monthly trends',0);
    legend('data input',...
	   'The ''trendline''',0);
    grid on; 
    dateaxis;
  ...
  %
  %
elseif issingle(f) % THE METHOD OF TAKING MONTHLY TRENDS (MONTHLY DATA ONLY!)
  Mf=floor(M/f);		% Whole number of years with data
  MM=ceil(M/f);			% One more year, covers rest of data
  x2=nan([MM*f-M,D]);		% Pad with NaNs
  y=[x;x2];			%
  ...
  if nargout==0 & D==1
    ...
    text(M+1,yl(end,1),num2str(squeeze(P(1,:,:,:)),'%5.2f'),'color','g','horizontalalignment','left');
    %plot(t,y,'.',tt,yy,'r-','linewidth',2);
    %legend('data input',...
    %	 'detrended and deseasoned series',...
    %	 'The ''trendline''/sequence of monthly trends',0);
